...
describe("GET /jobs", () => {
  ...
  test("filtering jobs correctly", async () => {
    ...
    resp = await request(app).get("/jobs").query({
      title: "1",
    });
    expect(resp.body).toEqual({
      jobs: [
    ...
    const resp2 = await request(app).get("/jobs").query({
      minSalary: 150,
    });
    ...
    const resp3 = await request(app).get("/jobs").query({
      hasEquity: true,
    });
    ...
    const resp4 = await request(app).get("/jobs").query({
      hasEquity: false,
    });
  ...
  test("fails: if other parameters are included other than title, minSalary and hasEquity", async () => {
...
describe("PATCH /jobs/:id", () => {
  ...
  test("not found for non-existent job", async () => {
    const resp = await request(app)
      .patch(`/jobs/0`)
      .send({
        title: "newUpdated",
